...
document.addEventListener('mousemove', function (event) {
    ...
    // Detectar cambios bruscos de dirección (curvas)
    if (Math.abs(direction - lastDirection) > Math.PI / 4) {
        curvatureDetected = true;
    } else {
        curvatureDetected = false;
    }

    // Detectar errores de movimiento (cambios bruscos de velocidad y aceleración)
    if (Math.abs(acceleration) > 5 || Math.abs(speed - lastSpeed) > 10) {
        errorMovement = true;
    } else {
        errorMovement = false;
    }
    ...
    if (Math.abs(direction) < 0.1 && distance < 10) {
        correction = true;
    }
...
document.getElementById('submit-button').addEventListener('click', () => {
    ...
    fetch('/Account/CaptureMouseDataModSVM', {
        method: 'POST',
        headers: {
